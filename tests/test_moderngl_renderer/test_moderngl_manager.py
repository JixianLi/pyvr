...
class TestModernGLManagerShaderManagement:
    ...
    def test_load_shaders_compilation_error(
        self, mock_moderngl_context, mock_shader_files
    ):
        """Test handling of shader compilation errors."""
        manager = ModernGLManager()

        # Mock compilation error
        mock_moderngl_context["ctx"].program.side_effect = Exception(
            "Compilation failed"
        )
...
class TestModernGLManagerVolumeTextureManagement:
    ...
    def test_create_volume_texture_4d_normals(
        self, mock_moderngl_context, sample_normal_data
    ):
...
class TestModernGLManagerTransferFunctionTextures:
    ...
    def test_create_rgba_transfer_function_texture(
        ...
        texture_unit = manager.create_rgba_transfer_function_texture(
            color_tf, opacity_tf
        )
    ...
    def test_create_rgba_texture_with_custom_size(
        self, mock_moderngl_context, mock_transfer_functions
    ):
...
class TestModernGLManagerTextureUnitManagement:
    ...
    def test_multiple_texture_allocations(
        self, mock_moderngl_context, mock_transfer_functions
    ):
        ...
        volume_unit = manager.create_volume_texture(
            np.random.rand(16, 16, 16).astype(np.float32)
        )
        rgba_unit = manager.create_rgba_transfer_function_texture(color_tf, opacity_tf)
        normal_unit = manager.create_normal_texture(
            np.random.rand(16, 16, 16, 3).astype(np.float32)
        )
...
class TestModernGLManagerErrorHandling:
    ...
    def test_moderngl_context_creation_failure(self):
        ...
        with patch(
            "moderngl.create_context", side_effect=Exception("Context creation failed")
        ):
    ...
    def test_normal_volume_wrong_channels(self, mock_moderngl_context):
        ...
        wrong_channels = np.random.random((10, 10, 10, 2)).astype(
            np.float32
    ...
    def test_rgba_texture_creation_edge_cases(
        self, mock_moderngl_context, mock_transfer_functions
    ):
