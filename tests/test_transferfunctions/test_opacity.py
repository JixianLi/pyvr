...
def test_peaks_validation():
    ...
    # Empty peaks list
    with pytest.raises(ValueError, match="At least one peak position must be specified"):
...
def test_validation():
    ...
    with pytest.raises(InvalidControlPointError, match="opacity .* outside valid range"):
