...
def test_camera_parameters_validation():
    ...
    Camera(target=np.array([0.0, 0.0, 0.0]), distance=1.0)
    ...
    # Zero vectors
    with pytest.raises(
        ValueError, match="init_pos must not be at the same location as target"
    ):
        Camera(target=np.array([1, 0, 0]), init_pos=np.array([1, 0, 0]))
...
def test_camera_parameters_copy():
    ...
    original = Camera(azimuth=np.pi / 4, elevation=np.pi / 6, distance=2.0)
...
def test_repr():
    ...
    params = Camera(azimuth=np.pi / 4, elevation=np.pi / 6, distance=2.5)
