...
def test_get_camera_pos_basic():
    ...
    pos, up = get_camera_pos(
        target=target, azimuth=0.0, elevation=0.0, roll=0.0, distance=3.0
    )
...
def test_get_camera_pos_rotation():
    ...
    pos1, up1 = get_camera_pos(
        target, azimuth=0.0, elevation=0.0, roll=0.0, distance=distance
    )
    pos2, up2 = get_camera_pos(
        target, azimuth=np.pi / 2, elevation=0.0, roll=0.0, distance=distance
    )
...
def test_get_camera_pos_edge_cases():
    ...
    pos, up = get_camera_pos(
        target=np.array([0.0, 0.0, 0.0]),
        azimuth=0.0,
        elevation=np.pi / 2,  # 90 degrees elevation
        roll=0.0,
        distance=3.0,
