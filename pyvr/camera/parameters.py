...
class CameraParameters:
    ...
    # Initial vectors
    init_pos: np.ndarray = field(default_factory=lambda: np.array([1.0, 0.0, 0.0], dtype=np.float32))
    init_up: np.ndarray = field(default_factory=lambda: np.array([0.0, 0.0, 1.0], dtype=np.float32))
    ...
    def validate(self) -> None:
        ...
        for param in angle_params:
            ...
            if abs(value) > 4 * np.pi:  # More than 2 full rotations
                print(f"Warning: {param} = {value:.3f} rad ({np.degrees(value):.1f}°) is unusually large")
    ...
    @classmethod
    def from_spherical(cls, target: np.ndarray, azimuth: float, elevation: float, 
                      roll: float, distance: float, **kwargs) -> 'CameraParameters':
        """
    ...
    @classmethod
    def front_view(cls, target: Optional[np.ndarray] = None, distance: float = 3.0) -> 'CameraParameters':
    ...
    @classmethod
    def side_view(cls, target: Optional[np.ndarray] = None, distance: float = 3.0) -> 'CameraParameters':
    ...
    @classmethod
    def top_view(cls, target: Optional[np.ndarray] = None, distance: float = 3.0) -> 'CameraParameters':
    ...
    @classmethod
    def isometric_view(cls, target: Optional[np.ndarray] = None, distance: float = 3.0) -> 'CameraParameters':
