...
class ColorTransferFunction(BaseTransferFunction):
    ...
    def __init__(
        self,
        control_points: Optional[List[Tuple[float, Tuple[float, float, float]]]] = None,
        lut_size: int = 256,
    ):
        """
    ...
    def _get_default_control_points(
        self,
    ...
    @classmethod
    def single_color(
        cls, color: Tuple[float, float, float], lut_size: int = 256
    ...
    def from_colormap(
        ...
        control_points = [
            (float(xi), tuple(map(float, rgb))) for xi, rgb in zip(x_mapped, colors)
        ]
    ...
    def __repr__(self) -> str:
        ...
        return (
            f"ColorTransferFunction({len(self.control_points)} control points, "
