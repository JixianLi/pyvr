...
class OpacityTransferFunction(BaseTransferFunction):
    ...
    @classmethod
    def linear(
        cls, low: float = 0.0, high: float = 1.0, lut_size: int = 256
    ...
    @classmethod
    def one_step(
        cls, step: float = 0.5, low: float = 0.0, high: float = 1.0, lut_size: int = 256
    ) -> "OpacityTransferFunction":
        """
        ...
        control_points = [(0.0, low), (step, low), (step + epsilon, high), (1.0, high)]
    ...
    def apply_to_array(self, scalar_array: np.ndarray) -> np.ndarray:
        ...
        scalars, opacities = zip(*self.control_points)
        return (
            np.interp(scalar_array.flatten(), scalars, opacities)
            .reshape(scalar_array.shape)
            .astype(np.float32)
        )
