# v0.3.3 Ray Marching Opacity Consistency + Interface Bug Fixes

**Release Date:** 2025-11-15
**Type:** Bug Fixes (Breaking Visual Change + Breaking Limitation)

## Overview

This release fixes three critical bugs:
...
3. **Light Linking Broken (Critical):** Camera-linked lighting didn't follow camera rotation due to coordinate system mismatch between camera (quaternion-based) and light (spherical coordinates). Fixed by implementing headlight mode (light at camera position).
...
- Breaking limitation for light linking fix - offset parameters (azimuth_offset, elevation_offset, distance_offset) are now ignored. Light is always positioned at camera (headlight mode). Behavior change makes feature work as intended, but removes offset capability.
...
- Created validation script: `docs/bugs/threading-crash/test_threading_fix.py`

**See:** `docs/plans/v0.3.3-ray-marching-consistency-phase-04.md` for detailed implementation plan

### Fixed: Light Linking Doesn't Follow Camera Rotation

**Issue:** When camera-linked lighting ('l' key) was enabled, the light didn't properly follow camera rotation. As the user rotated around the scene, the lighting direction would shift unexpectedly.

**Root Cause:** The camera and light used **incompatible coordinate systems**:
- **Camera**: Uses quaternion-based rotation from init_pos/init_up vectors (complex, gimbal-lock-free)
- **Light**: Used simple spherical coordinates (sin/cos formulas)

These calculations produced different positions even with the same azimuth/elevation values, causing camera and light to diverge by ~4.7 units during rotation.

**Location:** `pyvr/lighting/light.py:update_from_camera()` (before fix)

**Fix:** Implemented **headlight mode** - light positioned at camera's actual location:

```python
# Before (WRONG - spherical coordinates)
...
# Result: Wrong position, doesn't match camera

# After (CORRECT - use camera's actual position)
camera_pos, _ = camera.get_camera_vectors()
self.position = camera_pos.copy()
self.target = camera.target.copy()
# Result: Light at camera position (headlight/flashlight mode)
```

**Changes:**
- Light now positioned at camera's exact location using `camera.get_camera_vectors()`
- Light points toward camera target (scene center)
- Offset parameters (azimuth_offset, elevation_offset, distance_offset) stored but currently ignored
- Updated docstrings to reflect headlight mode behavior

**Impact:**
- **Critical bug fix** - makes light linking feature actually work
- **Behavior change** - light now truly follows camera (headlight mode)
- **Breaking limitation** - light offset functionality no longer works (offsets are ignored)
- **No API changes** - method signatures unchanged, fully backward compatible
- **Simpler behavior** - more intuitive than offset-based positioning

**Known Limitation:**
- Light offset parameters (azimuth_offset, elevation_offset, distance_offset) are accepted but currently ignored in headlight mode
- If you were using offsets to position light relative to camera, this will no longer work
- Future versions may restore offset functionality in camera's local coordinate space

**Testing:**
- Manual testing confirms light follows camera during rotation
- Light position verified to match camera position
- Lighting remains consistent from viewer's perspective
- Created analysis script: `docs/bugs/light-rotation/test_light_rotation.py`

**See:** `docs/bugs/light-rotation/BUG_ANALYSIS_light_rotation.md` for detailed analysis
...
### Light Linking Offset Functionality (Broken)

**What Broke:**
- Light offset parameters (azimuth_offset, elevation_offset, distance_offset) are now **ignored** in camera-linked lighting
- If you were using `Light.camera_linked(azimuth_offset=...)` or `light.link_to_camera(azimuth_offset=...)` to position light relative to camera, this will no longer work
- Light is now always positioned at camera location (headlight mode)

**Why:**
- Previous implementation used incompatible coordinate systems (camera: quaternion, light: spherical)
- Offsets didn't work correctly - light would drift away from expected position during rotation
- Headlight mode fixes the core bug but removes offset capability

**Workaround:**
- None currently available - offsets are not supported in headlight mode
- Future versions may restore offset functionality in camera's local coordinate space

**Impact:**
- Low - offset feature was broken anyway (light didn't follow camera correctly)
- Most users expect headlight mode when enabling camera-linked lighting
- API unchanged - offset parameters still accepted (just ignored)
